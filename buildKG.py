...
# # // 1. Set the minstep of 'ping6', 'httpd', 'nvram' to 1
...
# # //2. 
UNWIND ['ping6', 'httpd', 'nvram'] AS name
MATCH (start:Binary {name: name}), (end:Binary)
WHERE start <> end
WITH start, end
MATCH path = shortestPath((start)-[*1..]->(end))
WITH start, length(path) AS steps
WHERE steps IS NOT NULL
SET start.minstep = CASE 
  WHEN start.minstep IS NULL THEN steps
  WHEN steps < start.minstep THEN steps
  ELSE start.minstep
END
